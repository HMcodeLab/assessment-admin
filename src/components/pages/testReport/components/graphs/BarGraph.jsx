...
const CustomBarChart = ({ rankCount = {}, lowestRank, marksCount = {} }) => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="custom-tooltip" style={{ backgroundColor: '#fff', border: '1px solid #ccc', padding: '10px' }}>
          ...
          <p className="label">{`Marks : ${payload[0]?.payload?.marks || 'N/A'}`}</p>
  ...
  // Ensure rankCount and marksCount are objects
  const safeRankCount = rankCount && typeof rankCount === "object" ? rankCount : {};
  const safeMarksCount = marksCount && typeof marksCount === "object" ? marksCount : {};
  ...
  const marksData = labels.map((label) => safeMarksCount[label] || 0);

  // Create chart data for Recharts
  const chartData = labels.map((label, i) => ({
    name: `${label}`, 
    marks: marksData[i], // Using marksCount for marks
    rank: rankData[i]    // Using rankCount for rank
  }));
