...
const AdminProfile = () => {
  ...
  const fetchAdminDetails = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_SERVER_DOMAIN}/getAdmin`, {
        headers: {
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(`${process.env.REACT_APP_SERVER_DOMAIN}/updateAdmin`, formData, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${adminToken}`,
        },
  ...
  if (error) {
    return <div className="flex items-center justify-center min-h-screen"><div>{error}</div></div>;
  }
  ...
  return (
    <div className="min-h-screen flex justify-center bg-gray-100 py-[7vh]">
      <div className="bg-white shadow-lg rounded-lg p-8 max-w-lg w-full h-[70vh] py-12">
        <h1 className="text-3xl font-bold text-green-600 mb-6 text-center">Admin Profile</h1>
        {adminData && (
          <div>
            {isEditing ? (
              ...
              <div>
                ...
                <div className="mb-4">
                  <label className="text-xl font-semibold text-gray-700">Profile:</label>
                  <input 
                    type="text" 
                    name="profile" 
              ...
              <div>
                <div className="mb-4">
                  <p className="text-xl font-semibold text-gray-700"><strong>Name:</strong> {adminData.firstName} {adminData.lastName}</p>
                </div>
                <div className="mb-4">
                  <p className="text-xl font-semibold text-gray-700"><strong>Email:</strong> {adminData.email}</p>
                </div>
                <div className="mb-4">
                  <p className="text-xl font-semibold text-gray-700"><strong>Role:</strong> {adminData.role}</p>
                </div>
                <div className="mb-4">
                  <p className="text-xl font-semibold text-gray-700"><strong>Mobile:</strong> {adminData.mobile}</p>
                </div>
                <div className="mb-4">
                  <p className="text-xl font-semibold text-gray-700"><strong>Profile:</strong> {adminData.profile}</p>
                </div>
                <button 
